#!/usr/bin/env python3
"""
Direct Comparative Benchmark: Fast-FHIR vs fhir.resources

This script compares Fast-FHIR performance against fhir.resources by directly importing
core functions to avoid Pydantic version conflicts.
"""
...
# Try to import Fast-FHIR core functions directly
try:
    # Import core resource classes directly
    ...
    def deserialize_patient_direct(data):
        """Direct patient deserialization without Pydantic."""
        patient = Patient()
        ...
        patient.resource_type = data.get('resourceType', 'Patient')
    ...
    def deserialize_organization_direct(data):
        """Direct organization deserialization without Pydantic."""
        org = Organization()
        if isinstance(data, str):
            data = json.loads(data)
        
        org.id = data.get('id')
        org.resource_type = data.get('resourceType', 'Organization')
        org.active = data.get('active')
        org.name = data.get('name')
        org.type = data.get('type')
        
        return org
    
    def deserialize_care_plan_direct(data):
        """Direct care plan deserialization without Pydantic."""
        care_plan = CarePlan()
        if isinstance(data, str):
            data = json.loads(data)
        
        care_plan.id = data.get('id')
        care_plan.resource_type = data.get('resourceType', 'CarePlan')
        care_plan.status = data.get('status')
        care_plan.intent = data.get('intent')
        care_plan.title = data.get('title')
        care_plan.subject = data.get('subject')
...
def benchmark_fast_fhir(resources: List[Dict[str, Any]], resource_type: str) -> BenchmarkResult:
    ...
    if resource_type == "Patient":
        deserializer = deserialize_patient_direct
    elif resource_type == "Organization":
        deserializer = deserialize_organization_direct
    elif resource_type == "CarePlan":
        deserializer = deserialize_care_plan_direct
    else:
...
def run_comparative_benchmark(resource_counts: List[int] = None) -> Dict[str, List[BenchmarkResult]]:
    ...
    print("📝 Note: Fast-FHIR tested with direct deserialization (no Pydantic)")
    print("📝 Note: fhir.resources uses Pydantic v2 validation")
    print()
...
def main():
    ...
    print("- Fast-FHIR tested with direct deserialization (no validation overhead)")
    print("- fhir.resources uses full Pydantic v2 validation")
    print("- Results show actual performance on your system")
    
    print("\n💡 Notes:")
    print("- Fast-FHIR direct mode focuses on speed over validation")
    print("- fhir.resources provides comprehensive FHIR validation")
    print("- Choose based on your speed vs validation requirements")
    print("- Both libraries are suitable for production FHIR applications")
